...
- **Карточка участка** — SEO-путь `app/[district]/[settlement]/[slug]/page.tsx` (основной) и `app/uchastok/[slug]/page.tsx` (резервный slug-путь). Старые пути `/plots/[id]` сохранены как 301-редиректы.
- **Блог** (`app/blog/page.tsx`, `app/blog/[slug]/page.tsx`). Старый путь `/news/[id]` редиректит на `/blog/[id]`.
- **Публичные страницы**: `/faq`, `/legal`, `/privacy`, `/terms`, `/cookies`, `/about`, `/contacts`, `/geodesy`, `/luxury`, `/cadastral-discount`, `/minimalist`.
...
#### `app/[district]/[settlement]/[slug]/page.tsx` (основная карточка участка)
**SEO-оптимизированная страница земельного участка**:
- Динамический роут `/{район}/{поселок}/{slug}` (например, `/gurevskiy/dobroe/uchastok-5-sotok`)
- Карточка с полной информацией, галереей фото, кнопками «Позвонить», «Записаться», «Как проехать»
- JSON-LD (Product, BreadcrumbList) для Google Rich Snippets
- Похожие участки (`SimilarPlotsSlider`)
- 301-редиректы со старых URL-схем

#### `app/uchastok/[slug]/page.tsx` (резервный путь)
**Короткий URL по slug** — используется если район/посёлок не определены. Редиректит на полный SEO-путь, когда данные доступны.

#### `app/plots/[id]/page.tsx` (legacy-редирект)
**Permanent redirect (301)** со старого `/plots/{id}` на актуальный `/{district}/{settlement}/{slug}`. Индексация отключена (`robots: noindex, nofollow`). Работает через `buildPlotSeoPath` из `lib/utils.ts`.

#### `app/news/[id]/page.tsx` (legacy-редирект)
**Permanent redirect** со старого `/news/{id}` на `/blog/{id}`.

#### `app/blog/page.tsx` и `app/blog/[slug]/page.tsx`
**Блог / Новости**:
- Список статей с пагинацией
- Детальная страница с rich-text (TipTap)
- Внутренние перелинковки (похожие статьи, блок на главной и FAQ)
...
1. Проверить логи контейнера: `ssh root@85.198.85.239 "docker logs -f --tail 100 rkkland_web"`
2. Проверить логи в консоли браузера (детальное логирование включено)
3. Использовать `/admin/fix-password` для сброса пароля
4. Проверить environment variables на сервере: `/var/www/rkkland/.env` и `/var/www/rkkland/.env.docker`
